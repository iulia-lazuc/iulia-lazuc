# 👋 Hi, I’m Iulia

**I focus on what’s worth building. I translate market signals into AI products before a line of code is written.**
This is where I test, break and rebuild, before real development begins.

---

### 🧠 What I do
I design and prototype AI products that learn from *human experience*, not just data.  
Every build starts from observed behavior. Feedback loops, conversations, or community signals and ends with a solution users trust.

I care most about:
- **Strategic validation** → testing the “why” before the “how.”
- **Rapid iteration** → low-code + AI-first stacks (Claude, GPT, n8n, Supabase, Cursor).
- **Clarity over complexity** → turning insights into Product Development Strategies.

---

### ⚙️ Tech Stack & Tools
- **AI:** GPT / Claude / Perplexity / OpenAI function calling  
- **Automation:** n8n, Make  
- **Backend:** Supabase, Postgres, Hostinger
- **Frontend:** Vercel, Lovable
- **Integrations:** Meta API, Slack API, Notion API
- **Prototyping:** Cursor, Excalidraw, Gamma, Canva, Poppy

---

### 🧩 Projects (selected)
| Project | Description | Stack |
|----------|--------------|--------|
| **Exy 2.0** | Community-driven real-estate evaluator built on WhatsApp |  n8n, GPT, Perplexity, Claude, Meta API, Supabase, Cursor, Vercel, Lovable |
| **Exy 1.0** | AI lead management solution for real-estate agents | n8n, GPT, Meta API, Supabase, Cursor, Vercel, Lovable, Hostinger |
| **North Anchor Detector** | Automation that detects misalignment between strategy & development, OKRs & execution | Slack, Notion, Make, GPT |

---

### 💡 What I’m exploring
- Building human-feedback loops into LLM workflows  
- Market trust as a north star  
- Applying *community-led validation* to early-stage AI product design  

---

### 🤝 Let’s connect
- [LinkedIn](https://linkedin.com/in/iulia-lazuc)  
- [Website](https://realestatelabs.ro)  
- [Instagram](https://instagram.com/realestatelabs.ro)

---

> _Build with people_  
> — guiding principle for every prototype here.
